...
For the managed `pi-deep-work` package, run `/deep config` once for machine-wide model roles and `/deep init` in each trusted repository that needs `build` or `fix`. Initialization discovers supported root and nested package checks and behavior tests without executing them; `/deep init --refresh` previews exact commands, updates generated `auto.*` entries while preserving custom policy, and backs up the previous file outside the checkout. By personal convention, `config/git/ignore` keeps `.pi/pi-deep-work.json` machine-local. This overrides the package's recommended committed-policy trust model; force-add the file in repositories where the policy should be versioned and shared.
