...
### Rust
- Don't use qualified paths unless it is a convention, like don't write `std::time::Duration` or `std::future::Future`, import those.
- Don't use `pub(super)` or `pub(crate)` without a good reason. If you want something to be only accessaible within module, make it `pub` but don't export it outside of the module.
