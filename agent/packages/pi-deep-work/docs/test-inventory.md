...
- One registered `/deep` command with strict grammar and completion, including linked design revision, interactive resolution, and approved-design build handoff.
- No `registerTool`, manifest prompt, natural-language trigger, or child-session extension loading.
- Unforgeable `UserOrigin` required for start/resume/resolve/cancel/recover.
...
- Deterministic design Markdown, structured constraints/decisions/alternatives, bounded substantive-finding revision, complete review findings, same-repository revision lineage, and digest-validated approved-design handoff.
