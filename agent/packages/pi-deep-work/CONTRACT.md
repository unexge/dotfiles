...
`/deep resolve` accepts a completed same-repository `ChangesRequired` design, build, or fix, collects and confirms operator answers, and starts a new linked workflow. On design runs, `--accept` deterministically records every finding as an accepted limitation and starts the same linked revision without UI prompts; it does not create approval or bypass the fresh panel. The linked request binds the source run and summary artifact digest. Mutation-bearing continuations require the live checkout and durable source evidence to match the exact recorded checkpoint before fresh qualification and review.
