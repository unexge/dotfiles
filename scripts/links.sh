...
render_zellij_config() {
  ...
  if command -v nu >/dev/null 2>&1; then
    ...
    color="$(env -u ZELLIJ_LABEL -u ZELLIJ_LABEL_COLOR nu --no-config-file -c 'source-env ($nu.default-config-dir | path join "local.nu"); $env.ZELLIJ_LABEL_COLOR? | default "#cccac2" | print -n')"
  fi
